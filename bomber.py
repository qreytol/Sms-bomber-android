...
services_number = 0
def send_for_number_sms(aa,minutess):
    headers = {
    ...
    end = datetime.now() + timedelta(minutes=int(minutess))
...
def start(aa,minutess):
    st = Thread(target=send_for_number_sms,args=(aa,minutess,))
    st.start()
